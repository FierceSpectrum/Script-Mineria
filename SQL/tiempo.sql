...
FROM (
        SELECT DISTINCT
            ...
            END AS DATE_ID, CASE YEAR(O.OrderDate)
                WHEN '1996' THEN '2020'
                WHEN '1997' THEN '2021'
                WHEN '1998' THEN '2022'
...
SELECT DISTINCT
    ...
    CASE DATEPART(WEEKDAY, O.OrderDate)
        WHEN 1 THEN 'Domingo'
        WHEN 2 THEN 'Lunes'
        WHEN 3 THEN 'Martes'
        WHEN 4 THEN 'Miercoles'
        ...
        WHEN 7 THEN 'Sabado'
    END AS WEEK_DAY,
    DATEPART(DAYOFYEAR, O.OrderDate) AS YEAR_DAY,
...
SELECT DISTINCT
    ...
    YEAR(J.FECHA_PEDIDO) AS YEAR,
    MONTH(J.FECHA_PEDIDO) AS MOUNTH,
    DAY(J.FECHA_PEDIDO) AS DAY,
    CASE
        WHEN MONTH(J.FECHA_PEDIDO) IN (1, 2, 3) THEN 1
        WHEN MONTH(J.FECHA_PEDIDO) IN (4, 5, 6) THEN 2
        WHEN MONTH(J.FECHA_PEDIDO) IN (7, 8, 9) THEN 3
    ...
    CASE
        ...
        ELSE 3
    END AS QUARTER,
    DATEPART(WEEK, J.FECHA_PEDIDO) AS WEEK,
    CASE DATEPART(WEEKDAY, J.FECHA_PEDIDO)
        WHEN 1 THEN 'Domingo'
        WHEN 2 THEN 'Lunes'
        WHEN 3 THEN 'Martes'
        WHEN 4 THEN 'Miercoles'
        ...
        WHEN 7 THEN 'Sabado'
    END AS WEEK_DAY,
    DATEPART(DAYOFYEAR, J.FECHA_PEDIDO) AS YEAR_DAY,
